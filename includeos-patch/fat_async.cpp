...
namespace fs
{
  ...
  void FAT_::traverse(std::shared_ptr<Path> path, cluster_func callback, const Dirent* const start) const
  {
    ...
    [this, path, weak_next, callback] (uint32_t cluster)
    {
      ...
      int_ls(
        ...
        [name, dirents, next, callback] (error_t err, Dirvec_ptr ents)
        {
          ...
          for (auto& e : *ents)
          {
            if (UNLIKELY(iequals(e.name(), name) || (e.name().size() == 11 && iequals(e.name().substr(0, e.name().find_first_of(' ')) + "." + e.name().substr(8, e.name().find_first_of(' ') - 8), name))))
            {
  ...
  void FAT_::ls(const std::string& path, on_ls_func on_ls) const {
  ...
  void FAT_::ls(const Dirent& ent, on_ls_func on_ls) const
  {
  ...
  void FAT_::read(const Dirent& ent, uint64_t pos, uint64_t n, on_read_func callback) const
  {
  ...
  void FAT_::stat(Path_ptr path, on_stat_func func, const Dirent* const start) const
  {
    // manual lookup
  ...
  void FAT_::cstat(const std::string& strpath, on_stat_func func)
  {
    // cache lookup
