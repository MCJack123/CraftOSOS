...
#ifndef FS_VFS_PATCH_HPP
#define FS_VFS_PATCH_HPP
...
namespace fs {
  ...
  struct VFS {
    ...
    using Disk_map = std::map<Disk_key, fs::Disk__ptr>;
    ...
    using Disk_ptr_weak = std::weak_ptr<Disk_>;
    ...
    static fs::Disk__ptr& insert_disk(hw::Block_device& blk) {
      Disk__ptr ptr = std::make_shared<Disk_>(blk);
    ...
    static void insert_dirent(Disk_key disk_id, Path path, insert_dirent_delg fn) {
      ...
      if (not disk->fs_ready())
        throw Disk_::Err_not_mounted(std::string("Disk ") + std::to_string(disk_id) + " does not have an initialized file system");
