...
namespace fs
{
  FAT_::FAT_(hw::Block_device& dev)
    : device(dev), ls_cache(new std::map<uint32_t, dirvector>()) {
    
  }

  void FAT_::init(const void* base_sector) {
    ...
    FS_PRINT("Sectors in root dir: %u\n", root_dir_sectors);
    ...
    FS_PRINT("System ID: %.8s\n", bpb->system_id);
    // read in FAT
    this->fat_count = bpb->fa_tables;
    file_allocation_tables = (void**)calloc(bpb->fa_tables, sizeof(void*));
    for (int i = 0; i < /*bpb->fa_tables*/ 1; i++) {
      file_allocation_tables[i] = (void*)malloc(bpb->sectors_per_fat * bpb->bytes_per_sector);
      buffer_t data = device.read_sync(bpb->hidden_sectors + bpb->reserved_sectors + i*bpb->sectors_per_fat, bpb->sectors_per_fat);
      if (!data) os::panic("Could not read FAT\n");
      memcpy(this->file_allocation_tables[i], data->data(), bpb->sectors_per_fat * bpb->bytes_per_sector);
    }
  }

  void FAT_::init(uint64_t base, uint64_t size, on_init_func on_init)
  {
    this->lba_base = base;
    this->lba_size = size;
  ...
  bool FAT_::int_dirent(uint32_t sector, const void* data, dirvector& dirents) const
  {
    auto* root = (cl_dir*) data;
    bool  found_last = false;

    for (int i = 0; i < 16 * sectors_per_cluster; i++) {
      if (UNLIKELY(root[i].shortname[0] == 0x0)) {
        ...
        if (LIKELY(root[i].is_longname())) {
          ...
          if (L->is_last()) {
            ...
            if (i + total >= 16 * sectors_per_cluster) return false;
          ...
          dirname = D->attrib & 0x10 ? trim_right_copy(dirname) : trim_right_copy(dirname.substr(0, 8)) + "." + trim_right_copy(dirname.substr(8, 3));
          dirname.erase(std::transform(dirname.begin(), dirname.end(), dirname.begin(), [](char c)->char{return std::tolower(c);}));
