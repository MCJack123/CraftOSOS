...
#define IDE_ENABLE_READ
#define IDE_ENABLE_WRITE
...
IDE::buffer_t IDE::read_sync(block_t blk, size_t cnt)
{
  ...
  IDBG("IDE: Read (sync) called on %lu + %lu\n", blk, cnt);
  ...
  for (int x = 0; x < cnt; x++) {
    ...
    for (size_t i = 0; i < IDE::SECTOR_ARRAY; i++)
        data[x*IDE::SECTOR_ARRAY+i] = hw::inw(IDE_DATA);
  }
  return buffer;
#else
  return nullptr;
...
static void autoreg() {
  hw::PCI_manager::register_blk(PCI::VENDOR_INTEL, IDE_PRODUCT_ID, &IDE::new_instance);
};
